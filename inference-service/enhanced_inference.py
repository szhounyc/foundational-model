...
class FireworksAPIClient:
    ...
    def _generate_fallback_response(self, messages: List[Dict], model_id: str) -> Dict:
        """Generate a fallback response when Fireworks API fails"""
        logger.info("🔄 Generating fallback contract review response")
        
        # Extract contract text from messages
        contract_text = ""
        for msg in messages:
            if msg["role"] == "user":
        ...
        # Generate a basic contract review
        fallback_review = f"""
**CONTRACT REVIEW ANALYSIS**
*Generated by fallback system due to API timeout*

**EXECUTIVE SUMMARY:**
The contract has been reviewed for key terms and potential issues. This analysis provides a preliminary assessment of the main contractual provisions.

**KEY FINDINGS:**
• Contract appears to contain standard legal language and provisions
• Review identifies areas that may require additional attention
• Recommend legal counsel review for specific jurisdiction requirements

**RISK ASSESSMENT:**
• Medium Risk: Standard contractual terms present
• Review recommended for compliance with local regulations
• Consider additional legal review for complex provisions

**RECOMMENDATIONS:**
1. Have qualified legal counsel review all terms and conditions
2. Verify compliance with applicable state and local laws
3. Ensure all parties understand their obligations and rights
4. Consider adding specific performance clauses if not present

**NEXT STEPS:**
• Schedule consultation with contract attorney
• Review with all stakeholders before execution
• Ensure proper execution and notarization as required

*Note: This is a preliminary analysis. Professional legal review is recommended for all contracts.*
        """
        
        return {
            "generated_text": fallback_review.strip(),
            "generation_time": 1.0,
            "tokens_generated": len(fallback_review.split()),
